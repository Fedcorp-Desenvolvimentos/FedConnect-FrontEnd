...
`[E]` O select de instrutor da turma (`useInstrutores.js`) e as abas de local do calendário (`useCursoCipa.js`, `listarLocais`) leem listas fixas do backend; não há tela para mantê-las. `[P]` O dono pediu em 2026-09-08 uma área na Condomed onde o usuário cadastre palestrantes e locais (PA-029). Lado backend em `FedConnect-Back-End/specs/curso-cipa-cadastros/`.
...
- **QUANDO** abro `/condomed/cadastros` na aba Palestrantes, **ENTÃO** a interface **DEVE** listar os palestrantes com nome, registro (`MTE/UF número`), se tem assinatura e se está ativo, com filtro "mostrar inativos". `[P]` PA-029
- **QUANDO** clico em Novo ou Editar, **ENTÃO** o formulário **DEVE** pedir nome, título (pré-preenchido "Técnico em Segurança no Trabalho"), número e UF do registro MTE e a assinatura (PNG ou JPEG até 500 KB) com prévia da imagem. `[P]` PA-029
...
- **QUANDO** desativo um palestrante, **ENTÃO** ele **DEVE** sumir do select da turma e da importação, mas continuar visível no histórico das turmas antigas. `[P]` PA-029
...
- **QUANDO** abro a aba Locais, **ENTÃO** a interface **DEVE** listar nome, prédio, capacidade, unidade emissora, se compartilha a sala de reunião da agenda e se está ativo. `[P]` PA-029
- **QUANDO** crio ou edito um local, **ENTÃO** o formulário **DEVE** pedir nome, prédio, capacidade e unidade (select das unidades que o backend oferece) e a marca "compartilha a sala de reunião da agenda", explicando que só um local pode ter essa marca. `[P]` PA-029
...
Turma, importação e filtros passam a enviar ids de local e instrutor; a planilha modelo aceita os nomes. Os campos derivados (`local_nome`, `instrutor_nome`, `capacidade`, `acima_da_capacidade`) não mudam. `[E]` `FedConnect-Back-End/specs/curso-cipa-cadastros/` RNF-CIP-004
...
- PA-029: reversão dos cadastros fixos — quem pode cadastrar, excluir × desativar (espelha a questão de número 010 do registro do backend) — trava RF-CIP-006, RF-CIP-007
