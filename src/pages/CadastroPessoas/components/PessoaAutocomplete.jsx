...
const PessoaAutocomplete = ({ 
  ...
  // ✅ Encontra a pessoa selecionada (incluindo busca local)
  const selectedPessoa = resultados.find(p => 
    String(p.pessoa || p.PESSOA || p.codigo) === String(value)
  );

  // ✅ Mostra nome da pessoa se encontrada, senão mostra o código
  const displayValue = searchInput || selectedPessoa?.nome || selectedPessoa?.NOME || value || '';

  // ✅ Carrega pessoas automaticamente ao focar se houver valor
  useEffect(() => {
    if (isOpen && value && resultados.length === 0 && !loading) {
      buscarPessoasPorNome(value);
    }
  }, [isOpen, value, resultados.length, loading, buscarPessoasPorNome]);
      ...
      <div ref={wrapperRef} style={{ position: 'relative' }}>
        <S.InputWithButton>
          <S.FormInput
            ...
            disabled={disabled}
            $error={error}
            placeholder={loading ? 'Buscando pessoas...' : placeholder}
          />
          {value && (
            <S.IconSquareButton
              type="button"
