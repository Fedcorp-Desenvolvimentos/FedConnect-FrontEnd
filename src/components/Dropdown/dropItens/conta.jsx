    ...
    <div className="conta-container">
      <main className="conta-content">
        <div className="config-card-user">
          ...
          {mensagemSucesso && <div className="alert alert-sucesso">{mensagemSucesso}</div>}
          {mensagemErro && <div className="alert alert-erro">{mensagemErro}</div>}
              ...
              {usuariosPaginados.map((usuario) => (
                <tr key={usuario.id}>
                  ...
                  <td style={{ display: 'flex', gap: '0.5rem' }}>
                    <button className="btn-icon" onClick={() => abrirModalEdicao(usuario)} title="Editar usuário">
                      <i className="bi bi-pencil-square text-primary"></i>
                    </button>

                    {usuario.id !== usuarios?.[0]?.id && (
                      <button className="btn-icon" onClick={() => abrirModalExclusao(usuario)} title="Excluir usuário">
        ...
        {mostrarModalEdicao && (
          <div className="modal-overlay">
            <div className="modal-content">
              ...
              <label>Função:</label>
              <select value={nivelAcessoEditado} onChange={(e) => setNivelAcessoEditado(e.target.value)}>
